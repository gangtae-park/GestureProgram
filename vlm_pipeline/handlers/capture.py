...
def handle(captured_frame: np.ndarray, norm_points, gesture_name: str) -> np.ndarray:
    ...
    capture_anchor = target_anchor.compute(captured_frame, target.get("bbox"), target.get("mask_bool"))
    capture_response = {
        "name": matched_obj.get("name", ""),
        "object_id": matched_obj.get("id", ""),
        "message": "Capture object recognised.",
    }
    target_anchor.merge_into_response(capture_response, capture_anchor)
