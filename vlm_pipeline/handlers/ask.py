...
def _resolve_match(crop_bgr):
    """Mirror of search_find_info._resolve_match. Returns (object_dict | None, meta)."""
    db = object_db.get_db()
    ...
    try:
        emb = clip_matcher.embed_image(crop_bgr)
    except Exception as exc:
        print(f"[Ask][ERROR] embed failed: {exc}")
        return None, {"status": "embed_error", "error": str(exc)}

    match = clip_matcher.match_against_db(emb, db)
    if match is None:
...
def handle(captured_frame: np.ndarray, norm_points, gesture_name: str) -> np.ndarray:
    ...
    matched_obj, match_meta = _resolve_match(clip_crop)
