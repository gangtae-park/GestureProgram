...
def _fail(overlay, gesture_name, reason, target_meta, match_meta=None):
    ...
    print(f"[Anchor] gesture fail | {reason}")
    return overlay
...
def handle(captured_frame: np.ndarray, norm_points, gesture_name: str) -> np.ndarray:
    ...
    # Gaze -> soft Gaussian field; gaze_bbox kept for logging / crop fallbacks.
    ...
    # ---- YOLO segment overlapping gaze (gaze field vs object mask, soft IoU) ----
    ...
    if yolo_items:
        ...
        if idx >= 0 and overlap > 0:
            ...
            target_source = "YOLO"
            print(
                f"[Anchor][YOLO] target | class={chosen['class_name']} "
    ...
    cv2.putText(
        overlay,
        f"ANCHOR: {matched_obj.get('name','?')} (score={match_meta['score']:.2f})",
        (20, overlay.shape[0] - 30),
        cv2.FONT_HERSHEY_SIMPLEX, 0.6, config.TARGET_COLOR, 2, cv2.LINE_AA,
    )
    print(
        f"[Anchor] matched id={matched_obj['id']} "
