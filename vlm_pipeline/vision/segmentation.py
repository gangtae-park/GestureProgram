...
def load_yolo_model() -> bool:
    ...
    if YOLO is None:
        ...
        return False
    try:
        if "yoloe" in config.SEG_MODEL_PATH.lower():
            _yolo_model = YOLOE(config.SEG_MODEL_PATH)
            names = list(config.SEG_PROMPT_CLASSES)
            _yolo_model.set_classes(names, _yolo_model.get_text_pe(names))
            print(f"[YOLO] Loaded YOLOE model: {config.SEG_MODEL_PATH} | prompts={names}")
        else:
...
def run_yolo(captured_frame: np.ndarray):
    ...
    print(f"[TIMING][YOLO] predict {(_time.perf_counter() - _t0) * 1000:.0f} ms")
