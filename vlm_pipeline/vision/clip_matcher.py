...
def embed_image(bgr_image: np.ndarray) -> np.ndarray:
    ...
    out = feats[0].detach().cpu().numpy().astype(np.float32)
    print(f"[TIMING][CLIP] embed {(_time.perf_counter() - _t0) * 1000:.0f} ms")
    return out
...
def resolve_db_match(crop_bgr, log_study: bool = True):
    ...
    if score < config.CLIP_MATCH_MIN_SCORE:
        if log_study:
            study.log_event("obj_recognition", f"below_threshold score={score:.3f}")
        return None, {
    ...
    obj = match["object"]
    if log_study:
        study.log_event("obj_recognition", f"matched {obj['id']} score={score:.3f}")
    return obj, {
        "status": "matched",
        "object_id": obj["id"],
        "score": score,
