...
from .. import state
from ..unity import object_ui
from . import voice_pipeline
from ..unity.network import process_ask_question, send_vlm_result_to_unity
...
def _process_ask_transcript_async(transcript: str, request_id: str):
    ...
    from .. import study
    study.log_event("input_end", f"ask_transcript len={len((transcript or '').strip())}")
...
def _process_voice_command_async(payload: dict):
    ...
    transcript = str(payload.get("transcript") or "").strip()
    from .. import study
    study.log_event("input_end", f"voice_transcript len={len(transcript)}")
    if not transcript:
    ...
    # Same-instant gaze: state.latest_gaze_norm is already delay-aligned to
    # the adb frame content (GAZE_SCREEN_DELAY_S), so reading it in the same
    # breath as the frame gives THE single capture-moment gaze point -- the
    # same dot the live pane draws on this exact frame. This is the primary
    # gaze source; the Unity-sent trail/snapshot are only fallbacks.
    with state.gaze_lock:
        live_gaze_norm = state.latest_gaze_norm
        live_gaze_tracked = bool(state.latest_is_tracked)
    ...
    # Route through the voice pipeline: question-end frame + gaze -> YOLO
    # picks the gaze target and CLIP matches it against object_db -> ONE
    # multimodal GPT call carrying the frame, the gaze pixel, the DB fields
    # (authoritative when matched), and the verbatim transcript. The model
    # classifies the 7-way intent and answers grounded in the DB; the intent
    # picks Unity's result card.
    ...
    # The pipeline uses its LAST sample as the question-end gaze pixel; the
    # listen-start snapshot above is only the fallback when no trail arrived.
    trail_bl = _extract_gaze_trail(payload)

    intent, status = voice_pipeline.dispatch(
        ...
        request_id=request_id,
        live_gaze_norm=live_gaze_norm,
        live_gaze_tracked=live_gaze_tracked,
    )
    print(
