...
from ..gaze import headcomp
from ..gaze import ridge
...
def parse_packet(msg: str) -> dict:
    ...
    # STUDY packets can be as short as "STUDY,END", so they bypass the
    # generic minimum-length check below.
    if pt == "STUDY":
        return {"type": "STUDY", "parts": [p.strip() for p in parts[1:]]}
...
def udp_receiver_loop(sock: socket.socket):
    ...
    while not state.stop_event.is_set():
        ...
        # NOT for STUDY packets: those come from the researcher panel on
        # localhost, and adopting 127.0.0.1 as the "Unity host" would
        # redirect every VLM_RESULT (including the study countdown itself)
        ...
        if ptype == "GAZE":
            ...
            # Study gaze stream: mapped screen coords + raw dir + head quat,
            # recorded only while a study session is active (cheap no-op
            # otherwise -- this runs at 30 Hz).
            from .. import study
            study.log_gaze(
                tracked, mapped,
                (pkt["gx"], pkt["gy"], pkt["gz"]), pkt["head_quat"],
            )
            now = time.time()
            ...
            with state.gaze_lock:
                ...
                if (
                    ...
                    from .. import study
                    study.log_event("input_start", "gesture_pinch")
                elif evt == "READY":
                    ...
                    # step was removed) -- END alone triggers the DB-based
                    # translate lookup inside handlers/translate.handle().
                    ...
                    from .. import study
                    # Ask/Save are two-step referents: gesture recognition is
                    # only the midpoint (question / note dictation follows),
                    # so it logs as ask_point / save_point and the later
                    # transcript arrival is the real input_end.
                    _evt = {"Ask": "ask_point", "Save": "save_point"}.get(
                        end_name, "input_end")
                    study.log_event(_evt, f"gesture_recognized:{end_name}")
                    print(
            ...
            from .. import study
            _evt = {"Ask": "ask_point", "Save": "save_point"}.get(
                pkt["action"], "input_end")
            study.log_event(_evt, f"ui_action:{pkt['action']}/{pkt['object_id']}")
            print(
            ...
            from .. import study
            if study.handle_packet(pkt["parts"]) == "start":
                send_study_countdown_to_unity()
...
def send_vlm_result_to_unity(payload: dict):
    ...
    try:
        ...
        _is_save_ack = (
            payload.get("gesture") == "Save"
            and payload.get("stage") == "ack"
            and payload.get("status") == "ok"
        )
        if (payload.get("gesture") != "StudyCountdown"
                and payload.get("stage") != "object_recognized"
                and not _is_save_ack):
            # object_recognized is Ask's phase-1 (AskQuestionCard) and the
            # Save ack only opens the SaveNoteCard input UI; the study result
            # events track the final AskResultCard / the Save-button press
            # (Unity NoteManager sends that one) instead.
            from .. import study
            study.log_event(
                "result_sent", f"{payload.get('gesture')}/{payload.get('status')}"
            )
    except Exception as exc:
        print(f"[UNITY-SEND][ERROR] sendto failed: {exc}")


def send_study_countdown_to_unity():
    """Fire the head-locked 3-2-1 countdown on the headset at user-study
    session start. Unity's StudySessionController subscribes to
    VlmResultReceiver.OnResult and filters gesture == 'StudyCountdown'."""
    send_vlm_result_to_unity({
        "timestamp": time.strftime("%Y%m%d_%H%M%S"),
        "gesture": "StudyCountdown",
        "status": "ok",
        "response": {"name": "countdown"},
    })
...
def send_stream_end_to_unity(
    ...
    # Study log: for the two-phase Ask, the ANSWER stream end is the moment
    # the AskResultCard content is final -- that is Ask's result_sent.
    # (Translate's result_sent already fired on its stage='ocr' VLM_RESULT,
    # so only Ask logs here to avoid double counting.)
    if gesture == "Ask":
        from .. import study
        study.log_event("result_sent", f"Ask/{status}")
    print(
